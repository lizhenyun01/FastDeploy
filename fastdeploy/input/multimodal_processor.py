...
from fastdeploy.input.mm_processor import IDS_TYPE_FLAG, MultiModalDataProcessor
from fastdeploy.utils import data_processor_logger


class MultiModalProcessor(ErnieProcessor):
    ...
    def __init__(
        ...
        self.image_patch_id = self.data_processor.image_patch_id
    ...
    def _check_mm_limits(self, item):
        if isinstance(item, dict):
            ...
            # 请求包含messages
            mm_data = {"image_url": [], "video_url": []}

            for message in item:
                if isinstance(message.get("content"), list):
                    for part in message["content"]:
                        if part.get("type") == "image_url":
                            mm_data["image_url"].append(part)
                        elif part.get("type") == "video_url":
                            mm_data["video_url"].append(part)
    ...
    async def process_request_dict(self, request, max_model_len=None):
        ...
        if request.get("prompt"):
            ...
            audios = multimodal_data.get("audios", None)
            outputs = self.data_processor.text2ids(request["prompt"], images, videos, audios)
        ...
        request["prompt_token_ids_len"] = int(len(request["prompt_token_ids"]))
    ...
    def append_generated_tokens(self, multimodal_inputs, generated_token_ids):
        ...
        for i in range(num_tokens):
            multimodal_inputs["position_ids"].append([int(start + i)] * 3)
        multimodal_inputs["cur_position"] += num_tokens

    def pack_outputs(self, outs):
        # Stack or nullify image-related fields
        if not outs["image_feature_urls"]:
            outs["image_feature_urls"] = None
            outs["image_grid_thws"] = None
            outs["image_type_ids"] = None
        else:
            outs["image_feature_urls"] = outs["image_feature_urls"]
            outs["image_grid_thws"] = outs["image_grid_thws"]
            outs["image_type_ids"] = outs["image_type_ids"]

        if not outs["video_feature_urls"]:
            outs["video_feature_urls"] = None
            outs["video_grid_thws"] = None
            outs["video_type_ids"] = None
        else:
            outs["video_feature_urls"] = outs["video_feature_urls"]
            outs["video_grid_thws"] = outs["video_grid_thws"]
            outs["video_type_ids"] = outs["video_type_ids"]

        if not outs["audio_feature_urls"]:
            outs["audio_feature_urls"] = None
            outs["audio_grid_thws"] = None
            outs["audio_type_ids"] = None
        else:
            outs["audio_feature_urls"] = outs["audio_feature_urls"]
            outs["audio_grid_thws"] = outs["audio_grid_thws"]
            outs["audio_type_ids"] = outs["audio_type_ids"]

        outs["image_patch_id"] = self.image_patch_id
        outs["video_patch_id"] = self.video_patch_id
        outs["audio_patch_id"] = self.audio_patch_id
        # Convert lists to arrays
        outs["input_ids"] = outs["input_ids"]
        outs["token_type_ids"] = outs["token_type_ids"]
        outs["position_ids"] = outs["position_ids"]
        outs["cur_position"] = int(outs["cur_position"])
